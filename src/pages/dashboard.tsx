...
import { getMedicineStaffs } from 'apiSdk/medicine-staffs';
...
import { InsuranceProviderListPage as ShowCaseList } from 'pages/insurance-providers';

function DashboardPage() {
  const { session } = useSession();

  const medicineStaffStat = useSWR<PaginatedInterface<MedicineStaffInterface>>(
    () => `/medicine-staffs`,
    () => getMedicineStaffs({ limit: 1, offset: 0 }),
  );

  const insuranceProviderStat = useSWR<PaginatedInterface<InsuranceProviderInterface>>(
    () => `/insurance-providers`,
    () => getInsuranceProviders({ limit: 1, offset: 0 }),
  );

  const guestStat = useSWR<PaginatedInterface<GuestInterface>>(
    () => `/guests`,
    () => getGuests({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Medicine Staffs',
      result: medicineStaffStat?.data?.totalCount,
      isLoading: medicineStaffStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Insurance Providers',
      result: insuranceProviderStat?.data?.totalCount,
      isLoading: insuranceProviderStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Guests',
      result: guestStat?.data?.totalCount,
      isLoading: guestStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Insurance Providers
                </Text>
                <NextLink href={`insurance-providers`} passHref legacyBehavior>
                  <Button
